...
from pydantic import Field, IPvAnyAddress, conint
...
class ArkConnectionDetails(ArkModel):
    ...
    port: conint(ge=0, le=65535) = Field(description='Port to use for connection', default=22)
    connection_type: ArkConnectionType = Field(description='Type of connection', default=ArkConnectionType.SSH)
    credentials: Optional[ArkConnectionCredentials] = Field(description='Credentials to use for connection')
    connection_data: Optional[Union[ArkWinRMConnectionData, ArkSSHConnectionData]] = Field(
        description='Extra data to be used for connection'
    )
